...
@section('panel')
    <div class="row responsive-row">
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.send.money.history') }}?search={{ $user->username }}"
                variant="success" title="Total Send Money" :value="$widget['total_send_money']" icon="las la-calendar-day" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.payment.history') }}?search={{ $user->username }}" variant="primary"
                title="Total Payment" :value="$widget['total_payment']" icon="las la-calendar-alt" />
        </div>
     
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.bank.transfer.all') }}?search={{ $user->username }}"
                variant="primary" title="Total Bank Transfer" :value="$widget['total_bank_transfer']" icon="las la-calendar-check" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.microfinance.all') }}?search={{ $user->username }}"
                variant="success" title="Total Microfinance Payment" :value="$widget['total_microfinance_payment']" icon="las la-calendar-check" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.donation.all') }}?search={{ $user->username }}" variant="primary"
                title="Total Donation" :value="$widget['total_donation']" icon="las la-calendar-check" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.utility.bill.all') }}?search={{ $user->username }}"
                variant="success" title="Total Utility Bill" :value="$widget['total_utility_bill']" icon="las la-calendar-check" />
        </div>
        <div class="col-xxl-3 col-sm-6">
            <x-admin.ui.widget.four url="{{ route('admin.education.fee.all') }}?search={{ $user->username }}"
                variant="primary" title="Total Education Fee Payment" :value="$widget['total_education_fee']" icon="las la-calendar-check" />
        </div>
    </div>
                        ...
                        <div class="row g-0">
                            <div class="col-sm-6">
                                <div class="widget-card widget--success">
                                    <a href="{{ route('admin.report.user.transaction') }}?search={{ $user->username }}"
                            ...
                            <div class="col-sm-6">
                                <div class="widget-card widget--success">
                                    ...
                                    <div class="widget-card-left">
                                        ...
                                        <div class="widget-card-content">
                                            <p class="widget-title">@lang('Total Add Money')</p>
                                            <h6 class="widget-amount">
                            ...
                            <div class="col-sm-6">
                                <div class="widget-card widget--warning">
                                    <a href="{{ route('admin.withdraw.data.all') }}?search={{ $user->username }}"
                                        class="widget-card-link"></a>
                                    <div class="widget-card-left">
                                        <div class="widget-icon">
                                        ...
                                        <div class="widget-content">
                                            <p class="widget-title">@lang('Total CashOut')</p>
                                            <h6 class="widget-amount">
            ...
            <form action="{{ route('admin.users.update', [$user->id]) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center gap-3">
                        ...
                        <x-permission_check permission="update user">
                        ...
                        @forelse ($loginLogs as $loginLog)
                            <div class="login-history__item d-flex justify-content-between align-items-center">
                                <div class="login-history__item-content d-flex align-items-center gap-2">
                                    ...
                                    <div class="login-history__success">
    ...
    <x-admin.ui.modal id="userStatusModal">
        <x-admin.ui.modal.header>
            <div>
                ...
                @else
                    <small>
                        <span class=" text--success">@lang('Ban reason was'):</span>
...
@push('breadcrumb-plugins')
    <div class=" d-flex gap-2  flex-wrap">
        <x-permission_check permission="update user balance">
        ...
        <x-permission_check permission="ban user">
            ...
            @else
                <button type="button" class="flex-fill btn  btn--success" data-bs-toggle="modal"
                    data-bs-target="#userStatusModal">
                    <i class="las la-ban me-1"></i>@lang('Unban User')
                </button>
            @endif
        </x-permission_check>
        <x-permission_check permission="view user notifications">
            <a href="{{ route('admin.report.user.notification.history') }}?search={{ $user->username }}"
        ...
        @if ($user->kyc_data)
            <a href="{{ route('admin.users.kyc.details', $user->id) }}" target="_blank"
                class="flex-fill btn  btn--success">
